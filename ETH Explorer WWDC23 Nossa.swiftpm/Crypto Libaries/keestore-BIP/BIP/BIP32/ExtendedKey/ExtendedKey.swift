...
internal extension BIP32.ExtendedKey.KeyPair {
    ...
    static func masterKeyPair(
        ...
        guard let data = EllipticCurveKit.KeyPair<Secp256k1>(privateKeyHex: privateKey.key.hexRepresentation)?.publicKey.data.compressed
        else {
    ...
    func next(
        ...
        publicKey = EllipticCurveKit.KeyPair<Secp256k1>.init(private: .init(base64: privateKey)!)
            .publicKey.data.compressed.bytes
